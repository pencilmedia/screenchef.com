function Switcher() {
	...
	*/
	...
	this._createPrimaryTab = function(data) {
		...
		////Set 'home tab' to selected by default
		if (tab.id == "switcher_tab_home"){
			tab_middle.className = "active";
			this._primary_tab_selected = tab;
		}
		...
		// Make the text
		var tab_left = document.createElement("p");
		var tab_left_text = document.createTextNode(data.tab_text);
		tab_left.appendChild(tab_left_text);
		tab_middle.appendChild(tab_left);
	...
	*/
	...
	this._primaryTabSelect = function(params) {
		...
		//alert(this._primary_tab_selected.id);
		//alert(params.src_obj.id);
	}
