...
uxd = {
	...
	cardInit : function()
	{
		...
		$('.card_closure').on('click',uxd.cardClosure);
		uxd.onResize();
	...
	openRight : function()
	{
		...
		// Content URL
		var content_url = './assets/design_experience.html';
		$('#lt_pane').css({ "width": CLOSED_PANE_WIDTH+"%", opacity: "0.9"}).addClass('cursor').addClass('shadow');
		$('#ct_pane').css({ "width": CLOSED_PANE_WIDTH+"%", opacity: "0.7"}).addClass('cursor');
	...
	onResize : function()
	{
		...
		$('#ct_pane .content_wrapper').height( $('#ct_pane').height() - $('#ct_pane h2').height() - $('header').height() -110);
