...
{
	var numberOfOptionsToDisplay =2;
	var itemsList=$("#items");
	var $preview=$("#preview");
	var myItem;
	var option;
	...
	{
		myItem=DWPE_FormFactory.getDefaultArgsListForType("radiogroup");
		myItem.label="Value of the Radio button "+i;
		option=	$('<option></option>').html( myItem.label );
		option.data("obj",myItem);
		itemsList.append(option);
	}*/
	
	$("#editable-tree").data(  "default-object", DWPE_FormFactory.getDefaultArgsListForType("radiogroup") );
	$("#editable-tree").data(  "properties-sheet-template",	
								DWPE_FormFactory.getPropertySheetFields('radiogroup'));
