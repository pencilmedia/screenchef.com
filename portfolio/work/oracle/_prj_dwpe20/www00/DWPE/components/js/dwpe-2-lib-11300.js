...
{var tooltip=(override_tooltip&&override_tooltip.length>0)?"":icon.tooltip;var markup="<span class='icons "+iconId+" "+extension+"' ";if(icon.tooltip.length>0)
markup+=" title='"+tooltip+"' ";markup+="></span>";return markup;}}
...
{var item=props.items[index];item.type=button_style;item._class=_class;markup+=this.make(item,"toolbar-item");}
...
{this.id=this.getID();this._class="DWPE_Toolbar_Class";this.label="DWPE_Toolbar";this.width="100%";this.alignment="L";this.items=new Array();$(".dwpe-button").live('mousedown',function(event)
{var $item=$(this);var cmd=$item.attr('cmd');var parent_btn=$item.closest('.dwpe-button.RadioButtonGroup,.dwpe-button.ToggleButton')
if(parent_btn)
...
break;case"url":if(DWPE_Utilities.isDisabled($item))break;document.location=action[1];break;default:break;}}
...
DWPE_FormFactory_Class.prototype.generateLOVMarkup=function(args){var markup='';var help=(args.help)?' help="'+args.help+'"':"";var defaultStyle=(args.default_style)?' DefaultStyle="'+args.default_style+'"':"";markup+='<ux class="dwpe-lov';if(args.required)markup+=' '+args.required;if(args.state)markup+=' '+args.state;if(args.value)markup+=' '+args.value;markup+='"';markup+=defaultStyle;markup+=help;markup+='>';markup+='\r\n';markup+='\t'+DWPE_Base_Class.getArgsAsString(args);markup+='\r\n</ux>'
return markup;}
DWPE_FormFactory_Class.prototype.initializeLOV=function($ux)
{var objId=DWPE_Utilities.getUniqueId();$ux.attr('id',objId);if($ux.attr('DefaultStyle')=='list')
{$ux.append('<select><option>List Item</option><option>List Item</option><option>List Item</option></select>');if($ux.hasClass('disabled')){$ux.find("select").attr('disabled','disabled');}}else{$ux.append('<input />');$('#'+objId+' input').click(function(e){var $fld=$(this);if((e.layerX)<$fld.width()+7)return
...
{$ux.html('<span class="dwpe-lov read-only">'+$ux.find('select').val()+'</span>');return;}
if($ux.hasClass('required'))$ux.find("input").addClass('required');}
