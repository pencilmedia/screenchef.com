...
{tab="";newline="";}
if(compact)
{args._class=null;args.type=null;quote="'";if(args.items&&args.items.length==0)args.items=null;}
var str="";var indent="";for(i=0;i<depth;i++){indent+=tab;}
str+=indent+"{"+newline;var delim="";for(var propName in args)
{var obj=args[propName];switch(typeof obj)
...
localStr+=newline+indent+"]";str+=delim+localStr;break;case"number":str+=delim+indent+tab+quote+propName+quote+':'+quote+obj+quote;break;case"string":default:obj=obj.replace(/\'/g,"\\'");str+=delim+indent+tab+quote+propName+quote+':'+quote+obj+quote;break;}
delim=","+newline;}
...
{var $list=$("#"+listID);var $option=$("<option></option>");var args=DWPE_BaseFactory.getDefaultArgsListForType($list.attr('onAdd_AddType'));$option.text(args.label);$option.data("obj",args);$list.append($option);};DWPE_Utilities={};DWPE_Utilities.performAction=function(o)
{$callee=$(o)
if($callee.attr("target")=="_javascript")
DWPE_Utilities.exec($callee);else
DWPE_Utilities.loadURL($callee);}
DWPE_Utilities.loadURL=function(o)
{var $callee=$(o);if(DWPE_Utilities.isDisabled($callee))return;var url=$callee.attr("url");var target=$callee.attr("target");if(target=="_self")
{document.location=url;}
else
{window.open(url,'');}}
DWPE_Utilities.exec=function(o)
{var $callee=$(o);if(DWPE_Utilities.isDisabled($callee))return;var cmd=$callee.attr("cmd");try{$(document).data('context',$callee);eval(cmd);}
...
{if(!parentID)return true;console.log(parentID);var $parent=$($object.parent());console.log(parent);while($parent.attr('id')!=parentID&&$parent.attr('tag')!='body')
...
DWPE_Utilities.getIconMarkup=function(iconName,extension)
...
{iconId="";}
if(icon==null)
{iconId="";return"<span class='icons "+iconId+" "+extension+"' style='background-image:url("+iconName+")'></span>";}
else
{var markup="<span class='icons "+iconId+" "+extension+"' ";if(icon.tooltip.length>0)
markup+=" title='"+icon.tooltip+"' ";markup+="></span>";return markup;}}
...
{return'<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem illo inventore doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et <a class="open-window" href="javascript:void(0);" url="/DWPE/DWPE_SampleContent/sample_dialog.htm">open&nbsp;anchored&nbsp;window</a> architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur <a class="open-window" href="javascript:void(0);" url="/DWPE/DWPE_SampleContent/sample_tabbed_dialog.htm">open&nbsp;tabbed&nbsp;dialog</a> magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, <a class="open-window" href="javascript:void(0);" url="/DWPE/DWPE_SampleContent/sample_table_dialog.htm">open&nbsp;table&nbsp;dialog</a> qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit, sed quia non numquam eius modi tempora incidunt ut labore et dolore magnam aliquam <a class="open-window" href="javascript:void(0);" url="/DWPE/DWPE_SampleContent/sample_modal_dialog.htm">open&nbsp;modal&nbsp;dialog</a> quaerat voluptatem. Ut enim ad minima veniam, quis nostrum exercitationem ullam corporis suscipit laboriosam, nisi ut aliquid ex ea commodi consequatur? Quis autem vel eum iure reprehenderit qui in ea voluptate velit esse quam nihil molestiae consequatur, vel illum qui dolorem eum fugiat quo voluptas nulla pariatur?</p>';}
...
{case'dwpe-radiogroup':case'dwpe-checkbox':case'dwpe-prompt':case'dwpe-textarea':case'dwpe-textbox':case'dwpe-fileupload':break;case'dwpe-table':DWPE_TableFactory.make($parent);break;case'dwpe-tree':DWPE_TreeFactory.make($parent);break;case'dwpe-menu':DWPE_WindowFactory.make($parent);break;case'dwpe-singleselectchoicelist':case'dwpe-spinbox':case'dwpe-multiselectchoicelist':case'dwpe-multiselectlistbox':DWPE_FormFactory.make($parent);break;case'dwpe-horizontal-splitter':case'dwpe-vertical-splitter':case'dwpe-accordion':case'dwpe-tabset':case'dwpe-trainset':case'dwpe-header':case'dwpe-subheader':case'dwpe-subsubheader':DWPE_PanelFactory.make($parent);break;case'dwpe-carousel':DWPE_Carousel.make($parent);break;case'dwpe-toolbar':DWPE_Toolbar.make($parent);break;case'dwpe-toolbar':var args=ux_data[$parent.attr("data")];var obj=DWPE.make(args);obj.parent=$parent;obj.render();break;case'dwpe-button':case'dwpe-overflow-btn':case'dwpe-toolbar-resize':break;case'dwpe-window-container':DWPE_WindowFactory.make($parent);break;default:console.log($parent)
var args=ux_data[$parent.attr("id")];var obj=DWPE.make(args);obj.parent=$parent;obj.render();break;}})
...
DWPE_ButtonFactory_Class.prototype.makeClickableRegion=function(content)
{var newline="\r\n";tab="\t";var className=this.buttonClass;if(this.properties.state=="0")
className+=" Button_Disabled";else if(this.properties.selected=="1")
className+=" Button_Active";if(this.properties.live_events)
className+=" "+this.properties.live_events;var markup="<ux class='"+className+"' ";if(this.properties.tooltip)
markup+=" title='"+this.properties.tooltip+"' ";if(this.properties.command)
markup+=" cmd='"+DWPE_Utilities.makeJavascriptSafe(this.properties.command)+"'";markup+=">";markup+=newline+tab+content;markup+=DWPE_Base_Class.getArgsAsString(this.properties);markup+=newline+"</ux>";return markup;}
DWPE_ButtonFactory_Class.prototype.update=function()
{this.markup=this.make(this.properties);}
DWPE_ButtonFactory_Class.prototype.make=function(args)
{this.properties={};for(var argName in args)
{this.properties[argName]=args[argName];}
switch(args.type)
{case'TextButton':return this.makeTextButton(args);break;case'InlineSelector':return this.makeInlineSelectorButton(args);break;case'RadioButtonGroup':return this.makeRadioButtonGroup(args);break;case'IconicButton':return this.makeIconicButton(args);break;case'IconicTextButton':return this.makeIconicTextButton(args);break;case'ToggleButton':return this.makeToggleButton(args);break;case'ToolbarSeparator':return this.makeToolbarSeparator(args);break;default:args.label=this.formatString(args.label+" [could not resolve button.type]");return this.makeTextButton(args);break;}}
DWPE_ButtonFactory_Class.prototype.setDefaultValue=function(name,value)
{if(!this.properties[name])this.properties[name]=value;}
DWPE_ButtonFactory_Class.prototype.makeToggleButton=function(args)
{var _args={}
_args._class="DWPE_ButtonFactory_Class";_args.type="RadioButtonGroup";_args.button_style=args.button_style;_args.selected=args.selected;_args.state=args.state;_args.items=[];var item={}
item.label=this.formatString(args.label);item.url=args.url;item.target=args.target;item.tooltip=args.tooltip;item.iconId=args.iconId;_args.items.push(item);return DWPE_ButtonFactory.make(_args);}
DWPE_ButtonFactory_Class.prototype.makeRadioButtonGroup=function(args)
{var _className="dwpe-button RadioButtonGroup";_className+=(this.properties.state==0)?" Button_Disabled":"";var markup="<ux class='"+_className+"' style='padding:0px;margin:0px;' "+" onclick='DWPE_Toolbar_Class.toggle(event,this)'><ul>";var props=args;var button_style=props.button_style;var _class="DWPE_ButtonFactory_Class";var state=(props.state)?props.state:"1";var selected=parseInt(props.selected);for(var index in props.items)
{var args=props.items[index];args.type=button_style;args._class=_class;args.state=state;args.selected=(parseInt(index)+1==selected)?"1":"0";markup+=this.make(args);}
markup+="</ul>";markup+=DWPE_Base_Class.getArgsAsString(this.properties);markup+="</ux>";return markup;}
DWPE_ButtonFactory_Class.prototype.makeInlineSelectorButton=function(args)
{this.setDefaultValue("url","");this.setDefaultValue("target","");this.buttonClass="InlineSelectorInner";var outer_class="";var label="<span class='label'>"+this.formatString(this.properties.label)+"</span>";var icon=DWPE_Utilities.getIconMarkup(this.properties.iconId);switch(args.button_style)
{case"5":outer_class+="dwpe-button IconicButton InlineSelectorSplit";break;case"4":outer_class+="dwpe-button IconicButton InlineSelector";break;case"3":outer_class+="dwpe-button IconicButton InlineSelectorSplit";label="";break;case"2":outer_class+="dwpe-button IconicButton InlineSelector";label="";break;case"1":outer_class+="dwpe-button InlineSelectorSplit";icon="";break;case"0":default:outer_class+="dwpe-button InlineSelector";icon="";break;}
outer_class+=(args.state=="0")?" Button_Disabled":"";var markup="";markup+="<ux class='"+outer_class+"' onclick='DWPE_Toolbar_Class.showInlineSelectorMenu(event,this)' ";if(this.properties.cmd)
markup+=" cmd='"+DWPE_Utilities.makeJavascriptSafe(this.properties.cmd)+"'";if(this.properties.tooltip)
markup+=" title='"+this.properties.tooltip+"'";markup+=">";markup+=icon+label+"<span class='arw'>&nbsp;</span>";markup+=DWPE_Base_Class.getArgsAsString(this.properties);markup+="</ux>";return markup;}
DWPE_ButtonFactory_Class.prototype.generateTextButtonMarkup=function(args)
{this.properties=args;this.buttonClass="dwpe-button";var markup=this.makeClickableRegion(this.formatString(args.label));return markup;}
DWPE_ButtonFactory_Class.prototype.makeIconicButton=function(args)
{this.setDefaultValue("url","");this.setDefaultValue("target","");this.buttonClass="dwpe-button IconicButton";var markup=this.makeClickableRegion(DWPE_Utilities.getIconMarkup(this.properties.iconId));return markup;}
DWPE_ButtonFactory_Class.prototype.makeIconicTextButton=function(args)
{this.setDefaultValue("url","");this.setDefaultValue("target","");this.buttonClass="dwpe-button IconicButton";var markup=this.makeClickableRegion(DWPE_Utilities.getIconMarkup(this.properties.iconId)+' <span>'+this.formatString(this.properties.label)+'</span>');return markup;}
DWPE_ButtonFactory_Class.prototype.makeToolbarSeparator=function(args)
{return"<ux class='dwpe-button Separator'><span></span></ux>";}
...
{case'MenuButton':case 100:return{"_class":"DWPE_ButtonFactory_Class","type":"MenuButton","label":"MenuButton"+i,"items":[{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button 0","iconId":"add"},{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button 1","url":"#","target":"_self","iconId":"edit"},{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button 2","url":"#","target":"_self","iconId":"delete"}]};break;case'ToggleButton':case 6:return{"_class":"DWPE_ButtonFactory_Class","type":"ToggleButton","button_style":"IconicTextButton","label":"Toggle Button","url":"#","target":"_self","iconId":icon,"tooltip":"Toggle Button"};case'ToolbarSeparator':case 5:return{"_class":"DWPE_ButtonFactory_Class","type":"ToolbarSeparator","label":"Toolbar Separator"+i};break;case'InlineSelector':case 4:return{"_class":"DWPE_ButtonFactory_Class","type":"InlineSelector","label":"Inline Selector","tooltip":"Inline Selector","url":"#","target":"_self","iconId":icon};break;case'InlineSelectorSplit':case 4:return{"_class":"DWPE_ButtonFactory_Class","type":"InlineSelector","button_style":"5","label":"Sample Label","iconId":"Add","url":"#","target":"_self","state":"1"};break;case'RadioButtonGroup':case 3:return{"_class":"DWPE_ButtonFactory_Class","type":"RadioButtonGroup","button_style":"IconicButton","label":"Radio Button Group"+i,"items":[{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button 0","iconId":"add","tooltip":"Radio Button Group"},{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button 1","url":"#","target":"_self","iconId":"edit","tooltip":"Radio Button Group"},{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button 2","url":"#","target":"_self","iconId":"delete","tooltip":"Radio Button Group"}]};break;case'IconicTextButton':case 2:return{"_class":"DWPE_ButtonFactory_Class","type":"IconicTextButton","label":"Iconic TB","url":"#","target":"_self","iconId":icon,"tooltip":"Iconic Text Button"};break;case'IconicButton':case 1:return{"_class":"DWPE_ButtonFactory_Class","type":"IconicButton","label":"Iconic Button "+i,"url":"#","target":"_self","iconId":icon,"tooltip":"Iconic Button"};break;case'TextButton':case 0:default:return{"_class":"DWPE_ButtonFactory_Class","type":"TextButton","label":"Text Button "+i,"url":"#","iconId":icon,"target":"_self","tooltip":"Text Button"};break;}}
...
{case'MenuButton':return[{type:"hidden",id:"_class",name:"_class",value:"DWPE_ButtonFactory"},{type:"hidden",id:"type",name:"type",value:"MenuButton"},{type:"text",id:"label",name:"Label",value:""},{type:"text",id:"cmd",name:"Command",value:"",help:"(javascript::alert('hi') or url::http://oracle.com)"},{type:"text",id:"iconId",name:"Icon ID",value:""}];break;case'IconicTextButton':case'IconicButton':case'TextButton':return[{type:"hidden",id:"_class",name:"_class",value:"DWPE_ButtonFactory"},{type:"hidden",id:"type",name:"type",value:_type},{type:"text",id:"label",name:"Label",value:""},{type:"text",id:"tooltip",name:"Tooltip",value:""},{type:"text",id:"cmd",name:"Command",value:"",help:"(javascript::alert('hi') or url::http://oracle.com)"},{type:"text",id:"iconId",name:"Icon ID",value:""}];break;case'InlineSelector':case'ToggleButton':case'ToolbarSeparator':case'RadioButtonGroup':default:return[];break;}}
...
{this.id=this.getID();this._class="DWPE_Toolbar_Class";this.label="DWPE_Toolbar";this.width="100%";this.alignment="L";this.items=new Array();$("UX.dwpe-button").live('mousedown',function()
{var $item=$(this);var cmd=$item.attr('cmd');if(cmd)
{var action=cmd.split("::");switch(action[0].toLowerCase())
{case"javascript":var command=action[1];if(DWPE_Utilities.isDisabled($item))break;try{$(document).data('context',$item);eval(command);}
catch(e){alert("error in command format");}
...
{var markup="<ux class='dwpe-toolbar'>";markup+="<ul>";var len=this.properties.items.length;for(var index=0;index<len;index++)
{var myItemArgs=this.properties.items[index];markup+=DWPE_ButtonFactory.make(myItemArgs);}
markup+="<ux class='dwpe-overflow-btn aft'></ux>";markup+="</ul>";var resizable=(this.properties.resizable=="0")?" style='display:none' ":"";markup+="<div class='dwpe-toolbar-resize' "+resizable+" ></div>";markup+="</ux>";this.markup=markup;}
...
{var markup="<ux class='dwpe-toolbar'>";markup+="<ul>";var len=args.items.length;for(var index=0;index<len;index++)
{var myItemArgs=args.items[index];markup+=DWPE_ButtonFactory.make(myItemArgs);}
markup+="<ux class='dwpe-overflow-btn aft'></ux>";markup+="</ul>";var resizable=(args.resizable=="0")?" style='display:none' ":"";markup+="<div class='dwpe-toolbar-resize' "+resizable+" ></div>";markup+="</ux>";return markup;}
DWPE_Toolbar_Class.prototype.make=function($ux)
{$ux.attr('id',DWPE_Utilities.getUniqueId());var w=($ux.attr("width"))?$ux.attr("width"):"100%";$ux.data('size',w);var $overflow_btn=$ux.find("UX.dwpe-overflow-btn.aft:first");$overflow_btn.click(function(e)
{DWPE_Toolbar_Class.handleOverflowClick($ux);});if(this.properties.resizable=="1")
{$resizer=$ux.find(".dwpe-toolbar-resize:first");$resizer.draggable({axis:'x',start:function(event,ui){},stop:function(event,ui)
{var delta=ui.position.left;var newWidth=$ux.width()+delta;$ux.data("size",newWidth+"px");ui.helper.css({"left":"0px","top":"0px"});DWPE_Toolbar_Class.manageToolbarOverflow($ux);}});}
$(window).bind('dwpe-component-resize',function(e,parentID)
...
{DWPE_Window.closeWindowGroup($ux.attr('id')+"-overflow");var $overflow_btn=$ux.find(">UL>UX.dwpe-overflow-btn.aft:first");$overflow_btn.show();var pW=$ux.parent().width();var size=$ux.data('size');size=(size)?size:"100%";if(size.indexOf('%')>-1)
{size=(parseInt(size)/100)*pW;}
else
{size=parseInt(size);}
$ux.width(size);var $buttons=$ux.find(">UL>UX.dwpe-button");$buttons.hide();var w=0;w+=$overflow_btn.outerWidth(true);w+=2;var overflown=false;$buttons.each(function(i)
...
{var $overflow_btn=$ux.find("UX.dwpe-overflow-btn.aft:first");var menu_id=$ux.attr('id')+"-overflow";var $menu=DWPE_Window.addWindow($overflow_btn,menu_id,"overflow-menu",menu_id);var $menu_body=$("#"+menu_id+"-body");$ux.find(">UL>UX.dwpe-button:hidden").each
(function()
...
{$ux.find(">UL>UX.dwpe-button:hidden").each
(function(j)
{$(this).remove();});$toolbar_body=$ux.find(">UL");$menu_body.find(">UX").each
(function(j)
...
{var group=$(_group);if(DWPE_Utilities.isDisabled(group))return;var target=event.originalTarget;while(target.tagName!="UX")
{target=target.parentNode;}
group.find("UX").each(function(i)
{var o=$(this);if(this==target)o.toggleClass("Button_Active");else o.removeClass("Button_Active");})}
DWPE_Toolbar_Class.showInlineSelectorMenu=function(event,callee)
{var $e=$(event);event.stopPropagation();if(DWPE_Utilities.isDisabled(callee))return;var $callee=$(callee);var drop_down_width=17;if($callee.hasClass("InlineSelectorSplit")&&!$(event.originalTarget).hasClass("arw"))
{DWPE_Utilities.performAction($callee);}
else
{var menu_id=$callee.attr("id")+"-menu";var group=null;var menu=DWPE_Window.addWindow($callee,menu_id,"menu",null);var $overflowMenuBody=$("#"+menu_id+"-body");$overflowMenuBody.append("<li>New</li><li>Edit</li><li>Open</li><li>Save</li><li>Save As</li><li>Delete</li>");}};DWPE_PanelFactory_Class.prototype=new DWPE_Base_Class();DWPE_PanelFactory_Class.constructor=DWPE_PanelFactory_Class;DWPE_PanelFactory_Class.prototype._super=DWPE_Base_Class.prototype;function DWPE_PanelFactory_Class()
...
args.label+
definition;markup+=DWPE_Base_Class.getArgsAsString(args);markup+="</ux>";return markup;}
...
DWPE_FormFactory_Class.prototype.generateSpinBoxSpinner=function(objId)
{var spinnerMarkup='';spinnerMarkup='<table cellspacing="0" cellpadding="0" border="0">\r\n';spinnerMarkup+=' <tr>\r\n';spinnerMarkup+='  <td rowspan="2"><input class="number"/></td>\r\n';spinnerMarkup+='  <td><a class="up" onclick="DWPE_FormFactory.spinnerOnClick(\''+objId+'\',\'up\');"><span></span></a></td>\r\n';spinnerMarkup+=' </tr>\r\n';spinnerMarkup+=' <tr>\r\n';spinnerMarkup+='  <td><a class="down" onclick="DWPE_FormFactory.spinnerOnClick(\''+objId+'\',\'down\');"><span></span></a></td>\r\n';spinnerMarkup+=' </tr>\r\n';spinnerMarkup+='</table>\r\n';return spinnerMarkup;}
...
{var objId=DWPE_Utilities.getUniqueId();$ux.append(this.generateSpinBoxSpinner(objId));var $fld=$ux.find("input")
...
if($ux.attr('current'))$fld.val(curVal);if($ux.attr('help'))$fld.attr('help',$ux.attr('help'));if($ux.hasClass('disabled')||$ux.hasClass('read-only'))$fld.attr('disabled','disabled');if($ux.hasClass('required'))$fld.addClass('required');}
...
DWPE_FormFactory_Class.msclCheckBoxOnClick=function(objId){var $ux=$('#'+objId).closest("ux").data("menu");var selectedItems='';$ux.find("li").removeClass('selected');$ux.find("INPUT:checkbox:checked").each(function(){var $fld=$(this);$fld.closest('div.mscl-listitems li').addClass('selected');selectedItems+=($fld.parent().text())+'; ';});var lastValue=selectedItems.slice(0,selectedItems.length-2);$('#'+objId+'_items').val(lastValue);$ux.data('mscl_last_value',lastValue);$ux.find(".mcsl-check-all").attr('checked',false);}
...
{this._class="DWPE_WindowFactory_Class";$("UX.dwpe-window-container>DIV.window>DIV.top").live('mouseover',function(){var $win_title=$(this);if(!$win_title.data("init"))
{$win_title.data("init",true);var $win_container=$win_title.closest('UX.dwpe-window-container');var $win;if($win_container.hasClass("modal"))
...
return false;});$("UX.dwpe-menu LI").live('click',function(event,ui)
...
return false;});return this;}
DWPE_WindowFactory=new DWPE_WindowFactory_Class();DWPE_WindowFactory_Class.prototype.menuScrollThreshold=14;DWPE_WindowFactory_Class.prototype.update=function()
{this.markup=this.make(this.properties);}
...
{var markup="";markup+="<ux class='dwpe-note-window-container note-anchored'>";markup+="<div class='cone left'></div>"
...
{var markup="";markup+="<ux class='dwpe-window-container also-resize %POSITION%  %MODE%'>";markup+="\r\t<div class='window %TYPE% %RESIZABLE%'>";markup+="\r\t<div class='top'>";markup+="\r\t\t<div class='left'>";markup+="\r\t\t\t %ICON% %TITLE%";markup+="</div>";markup+="\r\t\t<div class='right'>";markup+=DWPE_Utilities.getIconMarkup('help','');markup+="&nbsp;"
markup+=DWPE_Utilities.getIconMarkup('closetab','close-window');markup+="\r\t\t</div>";markup+="\r\t\t</div>";markup+="\r\t<div class='middle also-resize WindowRegion'>";markup+="\r\t\t\t %CONTENT%";markup+="\r\t</div>";markup+="\r\t<div class='bottom'>";markup+="\r\t\t<div class='left'>";markup+="\r\t\t</div>";markup+="\r\t\t<div class='right'>";markup+="%BUTTON-1%";markup+="%BUTTON-2%";markup+="%RESIZER%";markup+="\r\t\t</div>";markup+="\r\t</div>";markup+="\r\t</div>";markup+="\r</ux>";return markup;}
DWPE_WindowFactory_Class.prototype.messageListTemplate=function()
{var markup="";markup+="<ux class='dwpe-window-container also-resize %POSITION%  %MODE%'>";markup+="\r\t<div class='window %TYPE% %RESIZABLE%'>";markup+="\r\t<div class='top'>";markup+="\r\t\t<div class='left'>";markup+="\r\t\t\t %ICON% %TITLE%";markup+="</div>";markup+="\r\t\t<div class='right'>";markup+=DWPE_Utilities.getIconMarkup('closetab','close-window');markup+="\r\t\t</div>";markup+="\r\t\t</div>";markup+="\r\t<div class='middle also-resize WindowRegion'>";markup+="\r\t\t\t %CONTENT%";markup+="\r\t</div>";markup+="\r\t<div class='bottom'>";markup+="\r\t\t<div class='left'>";markup+="\r\t\t</div>";markup+="\r\t\t<div class='right'>";markup+="%BUTTON-1%";markup+="%BUTTON-2%";markup+="%RESIZER%";markup+="\r\t\t</div>";markup+="\r\t</div>";markup+="\r\t</div>";markup+="\r</ux>";return markup;}
DWPE_WindowFactory_Class.prototype.generateDialogMarkup=function(args)
{var markup=this.dialogTemplate();markup=markup.replace('%TYPE%',args.type).replace('%RESIZABLE%',args.resizable).replace('%POSITION%',args.position).replace('%TITLE%',args.title).replace('%CONTENT%',DWPE_Utilities.getSampleContent('short')).replace('%ICON%',"").replace('%MODE%',args.mode);var btn=DWPE_ButtonFactory.getDefaultArgsListForType('TextButton');btn.label="OK";btn.live_events="close-window";markup=markup.replace('%BUTTON-1%',DWPE_ButtonFactory.make(btn));btn.label="Cancel";btn.live_events="close-window";markup=markup.replace('%BUTTON-2%',DWPE_ButtonFactory.make(btn));if(args.resizable=="resizable")
...
{var markup=this.messageListTemplate();markup=markup.replace('%TYPE%',args.type).replace('%RESIZABLE%',args.resizable).replace('%POSITION%',args.position).replace('%TITLE%',"Confirmation").replace('%CONTENT%',DWPE_Utilities.getSampleContent('confirmation-dialog')).replace('%ICON%',DWPE_Utilities.getIconMarkup('confirm')).replace('%MODE%',args.mode);var btn=DWPE_ButtonFactory.getDefaultArgsListForType('TextButton');btn.label="OK";btn.live_events="close-window";markup=markup.replace('%BUTTON-1%',DWPE_ButtonFactory.make(btn));btn.label="Cancel";btn.live_events="close-window";markup=markup.replace('%BUTTON-2%','');if(args.resizable=="resizable")
markup=markup.replace('%RESIZER%',"<div class='resizer'></div>");else
markup=markup.replace('%RESIZER%',"");return markup;}
DWPE_WindowFactory_Class.prototype.generateInfoDialogMarkup=function(args)
{var markup=this.messageListTemplate();markup=markup.replace('%TYPE%',args.type).replace('%RESIZABLE%',args.resizable).replace('%POSITION%',args.position).replace('%TITLE%',"Info").replace('%CONTENT%',DWPE_Utilities.getSampleContent('info-dialog')).replace('%ICON%',DWPE_Utilities.getIconMarkup('info')).replace('%MODE%',args.mode);var btn=DWPE_ButtonFactory.getDefaultArgsListForType('TextButton');btn.label="OK";btn.live_events="close-window";markup=markup.replace('%BUTTON-1%',DWPE_ButtonFactory.make(btn));btn.label="Cancel";btn.live_events="close-window";markup=markup.replace('%BUTTON-2%',"");if(args.resizable=="resizable")
markup=markup.replace('%RESIZER%',"<div class='resizer'></div>");else
markup=markup.replace('%RESIZER%',"");return markup;}
DWPE_WindowFactory_Class.prototype.generateWarningDialogMarkup=function(args)
{var markup=this.messageListTemplate();markup=markup.replace('%TYPE%',args.type).replace('%RESIZABLE%',args.resizable).replace('%POSITION%',args.position).replace('%TITLE%',"Warning").replace('%CONTENT%',DWPE_Utilities.getSampleContent('warning-dialog')).replace('%ICON%',DWPE_Utilities.getIconMarkup('warning')).replace('%MODE%',args.mode);var btn=DWPE_ButtonFactory.getDefaultArgsListForType('TextButton');btn.label="Yes";btn.live_events="close-window";markup=markup.replace('%BUTTON-1%',DWPE_ButtonFactory.make(btn));btn.label="No";btn.live_events="close-window";markup=markup.replace('%BUTTON-2%',DWPE_ButtonFactory.make(btn));if(args.resizable=="resizable")
markup=markup.replace('%RESIZER%',"<div class='resizer'></div>");else
markup=markup.replace('%RESIZER%',"");return markup;}
DWPE_WindowFactory_Class.prototype.generateErrorDialogMarkup=function(args)
{var markup=this.messageListTemplate();markup=markup.replace('%TYPE%',args.type).replace('%RESIZABLE%',args.resizable).replace('%POSITION%',args.position).replace('%TITLE%',"Error").replace('%CONTENT%',DWPE_Utilities.getSampleContent('error-dialog')).replace('%ICON%',DWPE_Utilities.getIconMarkup('error')).replace('%MODE%',args.mode);var btn=DWPE_ButtonFactory.getDefaultArgsListForType('TextButton');btn.label="OK";btn.live_events="close-window";markup=markup.replace('%BUTTON-1%',DWPE_ButtonFactory.make(btn));btn.label="Cancel";btn.live_events="close-window";markup=markup.replace('%BUTTON-2%',"");if(args.resizable=="resizable")
markup=markup.replace('%RESIZER%',"<div class='resizer'></div>");else
markup=markup.replace('%RESIZER%',"");return markup;}
...
{indent+="\t";}
markup+=indent+"<li";var _class=(item.item_type!="normal")?" "+item.item_type+" ":"";_class+=(item.selected)?" "+item.selected+" ":"";_class+=(item.state)?" "+item.state+" ":"";_class=(_class.length>0)?" class='"+_class+"'":"";var group=(item.group)?' group="'+item.group+'"':'';var command=(item.cmd)?' cmd="'+item.cmd+'"':'';var tooltip=(item.tooltip)?' title="'+item.tooltip+'"':'';markup+=_class+group+command+tooltip+">";switch(item.item_type)
...
{var $win=$(markup);$win.css('z-index',DWPE_WindowFactory.getNextZ());$("body").append($win);var L=0;var T=0;var pW=($win.parent()===$('body'))?$win.parent().width():$win.parent().outerWidth()-2;var pH=($win.parent()===$('body'))?$win.parent().height():$win.parent().outerHeight()-2;if($win.hasClass('centered')||$win.hasClass('modal'))
...
{var i="";var icon=DWPE_Utilities.getRandomIcon();switch(type)
{case'menu-item':return{"_class":"DWPE_WindowFactory_Class","type":"menu-item","label":"Menu Item "+i,"key":"","item_type":"normal"};break;case'menu-separator':return{"_class":"DWPE_WindowFactory_Class","type":"menu-item","label":"----------","key":"","item_type":"separator"};break;default:return{};}}
