<div id="Header" class="header">
<!--************************************
    HEADER
*************************************-->
<div class="header_wrapper">
	<div class="breadcrumb_wrapper">
        <ul class="breadcrumbs">
        	<li><a href="#">Search Results</a> &raquo;</li>
	        <li>SRC-00363</li>
		</ul>
	</div>
	<div class="column_one layout">
		<h2>SRC-00363</h2>
		<p><strong>Sourcing Project</strong> &bull; Electronics Annual Quote 2007 (#1) -- Infineon, ST, Freescale, Analog Device</p>
	</div>
	<div class="column_two">
		<h2>Open</h2>
		<p>&nbsp;</p>
	</div>
</div>

    <!--************************************
        OBJECT LEVEL CONTROLS
    *************************************-->
	<div class="object_level_controls">
        <form action="#">
            <a href="#" class="button" id="cmdChangeStatus"><span>Change Status<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
            <a href="#" class="button" id="cmdGlobalActions"><span>Actions<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
            
            <a href="#" class="button" id="cmdGlobalActions"><span>123<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
            <a class="button" id="cmdAutoNumber" href="#"><span><em class="iconic auto_number">&nbsp;</em><em class="dropdown">&nbsp;</em>&nbsp;</span></a>
        </form>
	</div>

    <!--************************************
        TABS
    *************************************-->
    <div class="tabs_container skin_default special_case">
        <ul class="tabs">
        <li class="first_tab"><a href="#"><span>General Information<em>&nbsp;</em></span></a></li>
        <li><a href="#"><span>Items<em>&nbsp;</em></span></a></li>
        <li><a href="#"><span>AML<em>&nbsp;</em></span></a></li>
        <li class="selected"><a href="#"><span>Changes<em>&nbsp;</em></span></a>
            <ul>
                <li class="selected"><a href="#">Item Master</a></li>
                <li><a href="#">Items</a></li>
                <li><a href="#">Unapplied Changes</a></li>
                <li><a href="#">Response</a></li>
            </ul>
        </li>
        <li><a href="#"><span>Analysis<em>&nbsp;</em></span></a></li>
        <li class="indicator_gray"><a href="#"><span>RFQs<em>&nbsp;</em></span></a></li>
        <li><a href="#"><span>Discussions<em>&nbsp;</em></span></a></li>
        <li><a href="#"><span>Attachments<em>&nbsp;</em></span></a></li>
        <li><a href="#"><span>History</span></a></li>
        </ul>
    </div>
    
    <!--************************************
        MESSAGES
    *************************************-->
	<div id="dms" class="message">
    	<p class="text">&nbsp;</p>
	</div>
</div>
<div id="PaneContent" class="frame_pinned">
	<div class="Object">
<!-- BEGIN: Object Content -->
        <!--************************************
            VIEW CONTROLS
        *************************************-->
        
        <div class="view_controls">
           	<p class="view_controls_status">Quantities calcuated as of: 25.Jan.2007 07:57:20 AM CST</p>
            <h4 class="view_controls_header">Item Master</h4>

            <p><label>View:</label>
                <select name="" style="vertical-align:middle">
                    <option value="">All</option>
                    <option value="">Added</option>
                    <option value="">Deleted</option>
                    <option value="">Modified</option>
                </select>
                <a href="#" class="button" id="cmdViewActions"><span>View Actions<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
            </p>
        </div>
        
        <!--************************************
            TABLE ACTIONS
        *************************************-->
        <div class="table_actions">
            <div class="column_one no_width">
                <p>
                    <a href="#" class="button"><span>Add</span></a>
                    <a href="#" class="button"><span>Delete</span></a>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="#" class="button" id="cmdPrepare"><span>Prepare<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
                    <a href="#" class="button" id="cmdQuote"><span>Quote<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="#" class="button" id="cmdMore"><span>More Actions<em class="dropdown">&nbsp;</em>&nbsp;</span></a>
                </p>
            </div>
            <div class="column_three">
                <p>
                    <a href="#" class="button"><span>Save</span></a>
                    <a href="#" class="button"><span>Cancel</span></a>
                </p>
            </div>
        </div>
        <div class="scrollTableDiv aml">
            <table class="aml" cellpadding="0" cellspacing="0">
                <tr>
                	<th colspan="9">&nbsp;</th>
                	<th colspan="2" class="center">Quantity Break 1</th>                    
                </tr>
                <tr class="header">
                	<td class="handle"></td>
                    <td class="w60">&nbsp;</td>
                    <td class="rfq w15"></td>
                    <td class="attachment w15"></td>
                    <td class="manufacturer_part w15"></td>
                    <td>Number</td>
                    <td>Description</td>
                    <td>Rev</td>
                    <td>Commodity/Mfr Na</td>
                    <td>Quantity</td>
                    <td>Target Cost</td>
                </tr>
				<tr>
                    <td class="handle"></td>
                    <td class="w60">
                        <span class="level_02">
                            <em class="bom">&nbsp;</em>
                        </span>
                    </td>
                    <td></td>
                    <td class="gray_dot"></td>
                    <td></td>
                    <td>25-165905-01</td>
                    <td>MAPPING PROC. PROG PLD</td>
                    <td>B</td>
                    <td></td>
                    <td>0</td>
                    <td>0.906</td>                
                </tr>
				<tr>
                    <td class="handle"></td>
                    <td class="w60">
                        <span class="level_00">
    	                    <em class="expand">&nbsp;</em>
    	                    <em class="part">&nbsp;</em>
                        </span>
                    </td>
                    <td class="gray_dot"></td>
                    <td></td>
                    <td class="gray_dot"></td>
                    <td>20-115438</td>
                    <td>TRAN T THY SC141 D</td>
                    <td>A</td>
                    <td></td>
                    <td>7,000</td>
                    <td>0.282</td>                
                </tr>
				<tr>
                    <td class="handle"></td>
                    <td class="w60">
                        <span class="level_00">
    	                    <em class="collapse">&nbsp;</em>
    	                    <em class="part">&nbsp;</em>
                        </span>
                    </td>
                    <td class="gray_dot"></td>
                    <td></td>
                    <td class="gray_dot"></td>
                    <td>25-123099</td>
                    <td>TRANS S NFT BSP295</td>
                    <td>A</td>
                    <td></td>
                    <td>25,000</td>
                    <td>0.26</td>                
                </tr>
				<tr>
                    <td class="handle"></td>
                    <td class="w60">
                        <span class="level_03">
                            <em class="manufacturer_part">&nbsp;</em>
                        </span>
                    </td>
                    <td class="gray_dot"></td>
                    <td class="gray_dot"></td>
                    <td></td>
                    <td>BSP295E6327</td>
                    <td></td>
                    <td></td>
                    <td>INFINEON</td>
                    <td>25,000</td>
                    <td></td>                
                </tr>
				<tr>
                    <td class="handle"></td>
                    <td class="w60">
                        <span class="level_03">
                            <em class="manufacturer_part">&nbsp;</em>
                        </span>
                    </td>
                    <td class="gray_dot"></td>
                    <td></td>
                    <td></td>
                    <td>BSP295E6327</td>
                    <td></td>
                    <td></td>
                    <td>INFINEON</td>
                    <td>25,000</td>
                    <td></td>                
                </tr>
				<tr>
                    <td class="handle"></td>
                    <td class="w60">
                        <span class="level_00">
    	                    <em class="expand">&nbsp;</em>
    	                    <em class="part">&nbsp;</em>
                        </span>
                    </td>
                    <td class="gray_dot"></td>
                    <td></td>
                    <td class="gray_dot"></td>
                    <td>20-115438</td>
                    <td>TRAN T THY SC141 D</td>
                    <td>A</td>
                    <td></td>
                    <td>7,000</td>
                    <td>0.282</td>                
                </tr>
            </table>
      </div>
		<? include($_SERVER['DOCUMENT_ROOT'] . "/includes/layouts/tables/table_footer.php"); ?>
      
      <div class="bom_demo_table_filler" id="bomTableFiller"></div>

<!-- END: Object Content -->
	</div>
</div>
<div id="palette_enclosure"></div>