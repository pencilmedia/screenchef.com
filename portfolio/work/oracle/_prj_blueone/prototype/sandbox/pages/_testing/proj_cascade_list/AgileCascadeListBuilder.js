...
function treeInit(treeDrawTarget, treeDataSource) {
	tree = new YAHOO.widget.AgileSingleSelectCascadeListTree(treeDrawTarget);
...
function traverseTreeData(data, attachNode) {
	for (var i=0; i<data.length; i++) {
		...
		if (typeof (data[i]) != 'undefined' && typeof (data[i]['children']) != 'undefined') {
			var tmp_node = new YAHOO.widget.AgileSingleSelectCascadeListNode(tmp_obj, attachNode, false, true);
			traverseTreeData(data[i]['children'], tmp_node);
		} else {
			var tmp_node = new YAHOO.widget.AgileSingleSelectCascadeListNode(tmp_obj, attachNode, false, false);
		}
	}
}
