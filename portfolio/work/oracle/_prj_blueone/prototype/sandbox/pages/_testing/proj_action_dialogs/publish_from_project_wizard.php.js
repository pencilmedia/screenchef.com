var wizard = 
{
	...
	showStep: function(stepNumber)
	{
		...
		{
			case 1:
				var title = 'Select File to Publish';
				var message = 'Select the Microft Project file to publish.  File must be in XML format.';
				$('#cmdBack,#cmdValidate, #cmdImport').addClass('disabled');
				
				break;
			case 2:
				var title = 'Publish As';
				var message = 'Specify how the content should be published.';
				break;
			case 3:
				var title = 'Map Resources';
				var message = 'Map Project file resources to application resources.  Double click in System User column to select a user from the system address book.';
				$('#tag01,#tag02,#tag03').toggleClass('expand');
				$('#tag01_content,#tag02_content,#tag03_content').toggle();
				$('#cmdBack, #cmdImport').removeClass('disabled');
				break;
			case 4:
