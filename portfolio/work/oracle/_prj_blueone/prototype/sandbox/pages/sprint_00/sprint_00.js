...
agile.ux.multilist =
{
	...
	setYUIControls : function()
	{
		...
		anim = new YAHOO.util.Anim('added_notice', { opacity: { to: 0 } }, .4, YAHOO.util.Easing.easeNone); 
		anim.onComplete.subscribe(resetAddedPill);
		
		paletteAnim = new YAHOO.util.Anim('palette_added_notice', { opacity: { to: 0 } }, .4, YAHOO.util.Easing.easeNone); 
		paletteAnim.onComplete.subscribe(resetAddedPill2);	
		...
		oACDS = new YAHOO.widget.DS_JSArray(ourData);
		...
		myDataTable = new YAHOO.widget.DataTable("palette_results",myColumnSet,myDataSource,{rowSingleSelect:true, scrollable:true});
	...
	setEventHandlers : function()
	{
		...
		addEvent("agileautocomplete2",	'click',	qFocus2);
		addEvent("cmdPalette",			'click', 	agile.ux.multilist.openPalette);
		addEvent("close_floater", 		'click', 	agile.ux.multilist.closePalette);
		addEvent("palette_query",		'keyup', 	filterResults);
		addEvent("multi_select_query",	'keydown', 	agile.ux.multilist.multiSelectKeyHandler);
		...
		addEvent("palette_results",		'keyup', 	doResultsEnter);
	},
	test:function(e)
	{
		alert(e.keyCode);
	...
	itemDragged: function(oArgs)
	{
		...
		if (isNewItem(listItemName))
		{
			...
			{
				$('added_notice').style.display = 'block';
				gfadeTimer = null;
...
function doResultsEnter(e)
{
	if (e.keyCode == 13)
	{
		...
		var x = $('palette_results').firstChild;
...
function isNewItem(o)
{
	...
	var isNewItem = true;
	var items = $("pill_results").getElementsByTagName("li");
	for (var i = 0; i < items.length; i++) {
		var str = items[i].innerHTML.toLowerCase();
...
DDTable.prototype.onDragEnter = function(e, id) {
    ...
    if (id == "selected_container") { // dont do this if id is the inital target
		 ...
		 $('palette_tooltip').style.backgroundColor="#8cd921";
